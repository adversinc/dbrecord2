...
class DbRecord2 {
  ...
  async init() {
    ...
    if (this._options.dbh) {
      ...
      this._dbh = await _MysqlDatabase.default.masterDbh();
  ...
  async commit() {
    ...
    const trxDbh = _MysqlDatabase.default.masterDbhRO(); //console.log("Comparing dbh:", this._dbh.cid, trxDbh? trxDbh.cid: undefined);
    ...
    const res = await this._dbh.queryAsync(sql, values); // During the first insert the ${_locatefield()} field will be empty, and,
  ...
  async _read(locateValue, byKey) {
    ...
    const rows = await this._dbh.queryAsync(`SELECT * FROM ${this._tableName} WHERE ${field}=? LIMIT 1`, [locateValue]);
  ...
  async _readByKey(keys, values) {
    ...
    const rows = await this._dbh.queryAsync(`SELECT * FROM ${this._tableName} WHERE ${fields} LIMIT 1`, values);
    return this._createFromRows(rows);
  }
  /**
  ...
  static async forEach(options, cb) {
    ...
    const _dbh = await _MysqlDatabase.default.masterDbh();
